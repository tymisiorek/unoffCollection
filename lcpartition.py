...
def new_tagNames(rawData, nameList, titleIndices):
    lineNumber = 0
    nameLines = []
    for text in rawData:
        doc = nlp(text)
        for ent in doc.ents:
            nameLines.append(text)
            if lineNumber < len(nameList) - 1:
                nextLine = nameList[lineNumber+1]
            else:
                nextLine = nameList[0]

            if lineNumber < len(nameList) - 2:
                twoLines = nameList[lineNumber+2]
            else:
                twoLines = nameList[0]
            
            titleIndices = checkStructure(text, nameList[lineNumber-1], str(ent.text), lineNumber, nameList, nameLines, titleIndices)
        lineNumber+=1 
    return titleIndices
...
def checkStructure(currentLine, previousLine, name, lineNumber, rawData, nameLines, titleIndices):
